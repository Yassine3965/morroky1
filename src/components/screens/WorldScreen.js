...
export default class WorldScreen {
  ...
  // Centralized state update and re-render trigger
  ...
  bindEvents() {
    ...
    const registerBtn = this.container.querySelector('#btn-register-merchant');
    ...
    const streetSelect = this.container.querySelector('#filter-street');
    const kissariaSelect = this.container.querySelector('#filter-kissaria');
    const alleySelect = this.container.querySelector('#filter-alley');
    ...
    streetSelect?.addEventListener('change', async (e) => {
      ...
      this._setState({ 
        filters: { streetId, kissariaId: '', alley: '' },
    ...
    kissariaSelect?.addEventListener('change', async (e) => {
      ...
      this._setState({ loading: true });
      const alleys = kissariaId ? await LocationManager.getOptions('alley', { streetId: this.state.filters.streetId, kissariaId }) : [];
      this._setState({ 
        filters: { ...this.state.filters, kissariaId, alley: '' },
    ...
    alleySelect?.addEventListener('change', async (e) => {
      const alley = e.target.value;
       this._setState({ 
        filters: { ...this.state.filters, alley },
        loading: true
      });
      await this.fetchMerchants();
    });
  ...
  async loadStreets() {
    ...
    this._setState({ options: { ...this.state.options, streets } });
  }
  ...
  async fetchMerchants() {
    ...
    try {
      ...
      this._setState({ merchants, loading: false });
    } catch (err) {
  ...
  template() {
    ...
    const renderStars = (rating) => {
      ...
      for (let i = 0; i < fullStars; i++) stars += '<span class="text-yellow-400 text-lg">★</span>';
      if (halfStar) stars += '<span class="text-yellow-400 text-lg">★</span>';
      for (let i = 0; i < emptyStars; i++) stars += '<span class="text-gray-300 text-lg">★</span>';
      return stars;
    };
    ...
    return `
      <div class="min-h-screen flex flex-col bg-gray-50 rtl">
        <header class="bg-white shadow-sm px-6 py-4 flex justify-between items-center sticky top-0 z-50">
            ...
            <button id="btn-back" class="text-sm text-gray-500 hover:text-morroky-red transition-colors">خروج</button>
        </header>
        ...
        <main class="flex-1 p-6">
            <div class="max-w-7xl mx-auto">
                <div class="glass-morphism rounded-3xl p-8 mb-8 text-center border-dashed border-2 border-gray-200">
                    ...
                    <p class="text-gray-600 max-w-2xl mx-auto">هذه هي الخريطة الرقمية للأسواق المغربية. استكشف محلات درب عمر الموثقة.</p>
                </div>

                <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-8">
                    <div class="flex flex-col md:flex-row items-center gap-4">
                        <div class="flex items-center gap-2 text-morroky-dark font-bold whitespace-nowrap"><span class="text-xl">🔍</span><span>تصفية حسب:</span></div>
                ...
                <div class="mb-10 min-h-[400px]">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div class="flex items-center gap-3">
                            <span class="bg-morroky-green/10 text-morroky-green px-3 py-1 rounded-full text-xs font-bold">${merchants.length} متجر متوفر</span>
                        </div>
                    </div>
                    ${loading ? `
                    ...
                    ` : merchants.length === 0 ? `
                        <div class="bg-white p-16 rounded-3xl text-center border-2 border-dashed border-gray-100">
                            ...
                            <p class="text-gray-500">حاول تغيير خيارات التصفية أو التأكد من صحة الاتصال.</p>
                        </div>
                    ` : `
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                            ${merchants.map(m => {
                                ...
                                const background_url = m.background_url || '';
                                    ...
                                    <div class="relative p-5 flex flex-col justify-between h-full">
                                        <div>
                                            <div class="flex items-center gap-4 mb-4">
                                                ...
                                                <div class="flex-1">
                                                    <div class="flex justify-between items-start">
                                                        ...
                                                        ${m.status === 'verified' ? `<div class="flex-shrink-0 bg-blue-100 text-blue-600 px-2 py-1 rounded-full text-xs font-bold border border-blue-200 flex items-center gap-1" title="متجر موثق"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg></div>` : ''}
                                                    </div>
                                            ...
                                            <div class="flex items-center gap-4 text-xs">
                                                <div class="flex items-center gap-1 text-green-600 font-bold"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg><span>${successful_sales} بيع ناجح</span></div>
                                                <div class="flex items-center gap-1 text-red-600 font-bold cursor-pointer hover:underline" data-merchant-id-rejections="${m.id}"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg><span>${unsuccessful_sales} بيع غير ناجح</span></div>
                                            </div>
                                        </div>
                                        <div class="mt-4">
                                            <button data-merchant-id-address="${m.id}" class="show-address-btn w-full text-center px-4 py-2 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition-colors text-sm font-bold">اظهر عنوان هذا المحل</button>
                                            <div id="address-container-${m.id}" class="text-sm text-gray-500 mt-2 font-medium" style="display: none;">${LocationManager.formatAddress(m.location || {})}</div>
                                        </div>
                                    </div>
                                </div>
                            `;
                ...
                <div class="flex justify-center mt-12 pb-12">
                    ...
                    <span>سجل محلك الآن</span><span class="text-xl">🚀</span>
                    </button>
                </div>
                ` : ''}
            </div>
