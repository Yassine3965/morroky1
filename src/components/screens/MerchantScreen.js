...
export default class MerchantScreen {
  ...
  bindEvents() {
    const homeButton = this.container.querySelector('#btn-home');
  ...
  async fetchMerchant(id) {
    try {
      ...
      this._setState({ loading: false }); // Set loading to false even on error
    }
  }
  ...
  template() {
      ...
      return `
        <div class="min-h-screen flex items-center justify-center bg-gray-50 rtl">
            <div class="text-center p-12 glass-morphism rounded-3xl max-w-md">
                ...
                <p class="text-gray-400 mb-6">ربما تم حذف هذا المتجر أو أن الرابط غير صحيح.</p>
    ...
    // Safely create WhatsApp link
    const whatsappNumber = m.phone ? m.phone.replace(/^0/, '212') : '';
    const whatsappLink = whatsappNumber ? `https://wa.me/${whatsappNumber}?text=${encodeURIComponent('مرحباً، أود الاستفسار عن منتج')}` : '#';
      ...
      <div class="min-h-screen bg-gray-50 rtl pb-20">
          ...
          <div class="max-w-6xl mx-auto px-6 -mt-32 relative z-10">
              <div class="bg-white rounded-3xl shadow-xl p-8 border border-gray-100 flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
                  ...
                  <a href="${whatsappLink}" target="_blank" class="w-full md:w-auto bg-[#25D366] text-white px-8 py-4 rounded-2xl font-black text-lg hover:scale-105 transition-transform shadow-lg shadow-green-200 flex items-center justify-center gap-3 mb-4 md:mb-0">
